        ...
        Service.getAllRooms().then(
            (result) => {
                result.forEach(element => {
                    if(typeof(lobby.rooms[element.id]) === typeof(new Room)){
                        lobby.rooms[element.id].name = element.name;
                        lobby.rooms[element.id].image = element.image;
                    }
                    else{
                        lobby.addRoom(element.id, element.name, element.image, element.messages);
                    }
                });
            });
        (error) =>{}
